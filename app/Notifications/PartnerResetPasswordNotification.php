...
class PartnerResetPasswordNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
		...
		//$role_id=\App\EstateOwners::where('id',$admin_id)->get();
		//$text=(count($role_id)>1) ? 'partner' : (($role_id[0]->role_id==2) ? 'partner' : 'admin'); 
        return (new MailMessage)
        ->line('You are receiving this email because we received a password reset request for your account.')
        ->action('Reset Password', url('partner_password/reset',$this->token))
