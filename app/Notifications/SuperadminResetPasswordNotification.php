...
class SuperadminResetPasswordNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
		...
		$role_id=\App\RoleAdmin::where('admin_id',$admin_id)->get();
		$text=(count($role_id)>1) ? 'superadmin' : (($role_id[0]->role_id==2) ? 'superadmin' : 'admin'); 
        return (new MailMessage)
        ->line('You are receiving this email because we received a password reset request for your account.')
        ->action('Reset Password', url($text.'_password/reset', $this->token))
