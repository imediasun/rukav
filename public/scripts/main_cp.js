...
function errorContainerShow(errorContainer) {
    errorContainer.show();
    // scrolling to top of errorContainer
    var errorContainerCoordinate = errorContainer.offset();
    window.scrollTo(errorContainerCoordinate.top, errorContainerCoordinate.left);
}
...
//    });
    ...
    function forceNumericInputValue (e) {
        ...
        if($(this).hasClass('rounded-numeric')) {
            var rawFieldValue = $.trim($(this).val());
            if(rawFieldValue) {
                fieldValue = Math.round(rawFieldValue.match(/([0-9]+[,\.][0-9]+|[0-9]+)/)[0]);  // 0.49 will be rounded down, 0.5 will be rounded up
            }
        } else {
            fieldValue = $.trim($(this).val().replace(/\D+/g, ''));
        }
    ...
    $(document).on('keyup', '.forced-numeric',function (e) {
    ...
    $.validator.setDefaults({
        ...
        highlight: function(element) {
            ...
            errorContainer.html(validationGeneralErrorText);
            errorContainerShow(errorContainer);
