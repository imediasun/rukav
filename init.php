...
$conf['module'] = [];
foreach (glob(BASE_DIR."/application/modules/*/Config/module.php") as $filePath) {
    $moduleConfig = include $filePath;
    if(is_array($moduleConfig)) {
        $conf['module'] = \App\modules\Core\Model\Utils::merge($conf['module'], $moduleConfig);
    }
}
...
$conn=mysql_connect($conf['mysql']['server'],$conf['mysql']['username'],$conf['mysql']['password']);
if(!$conn) {
    die("<font class='fehler'>Es konnte keine Verbindung hergestellt werden:</font> ".mysql_error());
}
$conn2=mysql_select_db($conf['mysql']['database']);
if(!$conn2) {
    die("<font class='fehler'>Die angegebene Datenbank(".$dbname.") konnte nicht gefunden werden:</font> ". mysql_error());
}
